...
const Claimed: FC<{
  ...
  console.log(isLoading, randomMonkeySrc, age, claimedOn);
  return (
...
const EligibleHandles = () => {
  const { data, error } = useSWR<EligibleFamClaimStatus[], Error>(
    `${getDomain()}/api/v2/fam/poap/eligible-fam`,
    fetchJsonSwr,
    { suspense: true },
  );
  ...
  // As long as we have suspense turned on we don't have to worry about the undefined state.
  // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
  const suspendedData = data!;

  const Row: FC<{ index: number; style: CSSProperties }> = ({
    style,
    index,
  }) => {
    const fam = suspendedData[index];
  ...
  return (
    <WidgetErrorBoundary title="claim poap">
      <WidgetBackground className="flex flex-col gap-y-8 ">
        <div className="grid grid-cols-[1fr_64px] md:grid-cols-[40px_2fr_1fr_64px] gap-x-4">
          <LabelText className="col-span-2">1559 Eligible Handles</LabelText>
          <LabelText className="hidden md:block">fam followers</LabelText>
          <LabelText>claimed</LabelText>
        </div>
        <FixedSizeList
          itemCount={suspendedData.length}
          height={400}
          itemSize={56}
          width="100%"
        >
