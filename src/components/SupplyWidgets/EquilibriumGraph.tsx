...
const EquilibriumGraph: FC<Props> = ({
  ...
  const renderingStart = useRef<Date>(new Date());
  ...
  const [options, setOptions] = useState<Highcharts.Options>({
    chart: {
      ...
      events: {
        render: function () {
          console.log(
            `rendering took ${Date.now() - renderingStart.current.getTime()}ms`,
          );
  ...
  useEffect(() => {
    ...
    console.log("isRendering", isRendering.current);

    if (
      options.series !== undefined &&
      _.isEqual(options.series[0], nextOptions.series![0])
    ) {
      // options haven't changed, nothing to do.
    } else {
      console.log(options, nextOptions);
      if (!isRendering.current) {
        console.log("not rendering, starting next render");
        renderingStart.current = new Date();
        isRendering.current = true;
        setOptions(nextOptions);
      } else {
        console.log("already rendering, not starting a new render");
      }
    }
  }, [
    height,
