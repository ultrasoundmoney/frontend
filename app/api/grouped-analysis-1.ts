...
import { NEA } from "../fp";
import type { BurnRecords } from "./burn-records";
...
export const useGroupedAnalysis1 = (): GroupedAnalysis1 | undefined => {
  ...
  const [latestGroupedAnalysis1, setLatestGroupedAnalysis1] =
    useState<GroupedAnalysis1>();

  useEffect(() => {
    if (useWebSockets) {
      if (dataWs !== undefined) {
        setLatestGroupedAnalysis1(dataWs);
      }
      return undefined;
    }

    if (data !== undefined) {
      setLatestGroupedAnalysis1(data);
      return undefined;
    }
